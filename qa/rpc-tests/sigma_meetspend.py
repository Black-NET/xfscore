...
def activate_sigma_spend(denoms, spendsize, exp_spends, exp_remints, xfsd):
    for denom in denoms:
        count, size = denom
        for i in range(count):
            xfsd.nodes[0].mint(size)
            xfsd.nodes[0].generate(6)

    myaddr = xfsd.nodes[0].listreceivedbyaddress(0, True)[0]['address']
    args = {myaddr: spendsize}
    txid = xfsd.nodes[0].spendmany("", args)
    xfsd.nodes[0].generate(2)

    # Should be checked spends
    spends = xfsd.nodes[0].listsigmaspends(0)
