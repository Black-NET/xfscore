...
/*
    At 15 signatures, 1/2 of the xfsnode network can be owned by
...
private:
    ...
    //track xfsnodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapXFSnodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageXFSnodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointXFSnode;
    std::vector<unsigned char> vchXFSnodeSignature;
    // local memory only
...
public:
    ...
    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointXFSnodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointXFSnode(outpointXFSnodeIn),
        vchXFSnodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointXFSnode);
        READWRITE(vchXFSnodeSignature);
    }
    ...
    COutPoint GetXFSnodeOutpoint() const { return outpointXFSnode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapXFSnodeVotes; // xfsnode outpoint - vote
...
public:
    ...
    bool HasXFSnodeVoted(const COutPoint& outpointXFSnodeIn) const;
    int CountVotes() const { return mapXFSnodeVotes.size(); }
...
public:
    ...
    bool HasXFSnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointXFSnodeIn);
