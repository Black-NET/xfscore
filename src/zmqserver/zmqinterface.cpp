...
CZMQPublisherInterface* CZMQPublisherInterface::Create()
{
    ...
    factories["pubxfsnodeupdate"] = CZMQAbstract::Create<CZMQXFSnodeTopic>;
    ...
    factories["pubxfsnodelist"] = CZMQAbstract::Create<CZMQXFSnodeListTopic>;
...
void CZMQPublisherInterface::NotifyXFSnodeList()
{
    for (std::list<CZMQAbstract*>::iterator i = notifiers.begin(); i!=notifiers.end(); )
    {
        CZMQAbstract *notifier = *i;
        if (notifier->NotifyXFSnodeList())
        {
            i++;
        }
        else
...
void CZMQPublisherInterface::UpdatedXFSnode(CXFSnode &xfsnode)
{
    for (std::list<CZMQAbstract*>::iterator i = notifiers.begin(); i!=notifiers.end(); )
    {
        CZMQAbstract *notifier = *i;
        if (notifier->NotifyXFSnodeUpdate(xfsnode))
        {
            i++;
        }
        else
