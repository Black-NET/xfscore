...
bool CZMQXFSnodeListEvent::NotifyXFSnodeList()
{
...
bool CZMQBlockEvent::NotifyBlock(const CBlockIndex *pindex){
    ...
    if(xfsnodeSync.GetBlockchainSynced() || pindex->nHeight%100==0){
        request.replace("data", pindex->ToJSON());
        Execute();
    }
...
bool CZMQXFSnodeEvent::NotifyXFSnodeUpdate(CXFSnode &xfsnode){
    request.replace("data", xfsnode.ToJSON());
    Execute();
