...
bool AppInit(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  xfsd [options]                     " + strprintf(_("Start %s Daemon"), _(PACKAGE_NAME)) + "\n";
    ...
    // bool fApi = false; 
        ...
        if(!xfsnodeConfig.read(strErr)) {
            fprintf(stderr,"Error reading xfsnode configuration file: %s\n", strErr.c_str());
            return false;
        }
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in xfsd anymore. Use the xfs-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "XFS server starting\n");
