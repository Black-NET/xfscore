...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given XFS address.\n"
            "\nArguments:\n"
            "1. \"indexaddress\"     (string, required) The XFS address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"indexaddress\", (string) The XFS address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: XFS address and we have full public key:
...
UniValue createsporkkeypair(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"indexaddr\" : \"indexaddr\", (string) The XFS public address \n"
...
UniValue insync(const UniValue& params, bool fHelp)
{
    ...
    if(strMode == "status") {
        ...
        objStatus.push_back(Pair("AssetID", xfsnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", xfsnodeSync.GetAssetName()));
        objStatus.push_back(Pair("Attempt", xfsnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", xfsnodeSync.GetBlockchainSynced()));
        objStatus.push_back(Pair("IsXFSnodeListSynced", xfsnodeSync.IsXFSnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", xfsnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", xfsnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", xfsnodeSync.IsFailed()));
        return objStatus;
    }
    ...
    if(strMode == "next")
    {
        ...
        return "sync updated to " + xfsnodeSync.GetAssetName();
    }
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are XFS addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) XFS address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"indexaddress\"  (string, required) The XFS address to use for the signature.\n"
