...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  \"xfsnode\" : {                  (json object) required xfsnode payee that must be included in the next block\n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) required amount to pay\n"
            "  },\n"
            "  \"xfsnode_payments_started\" :  true|false, (boolean) true, if xfsnode payments started\n"
            "  \"xfsnode_payments_enforced\" : true|false, (boolean) true, if xfsnode payments are enforced\n"
            "}\n"
    ...
    if (vNodes.empty() && chainActive.Tip()->nHeight > 100)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "XFS is not connected!");
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "XFS is downloading blocks...");

    if (!xfsnodeSync.IsSynced() && chainActive.Tip()->nHeight > 500)
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "XFS Core is syncing with network...");
...
//    const bool fPreSegWit = (THRESHOLD_ACTIVE != VersionBitsState(pindexPrev, consensusParams, Consensus::DEPLOYMENT_SEGWIT, versionbitscache));
    ...
    UniValue xfsnodeObj(UniValue::VOBJ);
    if(pblock->txoutXFSnode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutXFSnode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        xfsnodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        xfsnodeObj.push_back(Pair("script", HexStr(pblock->txoutXFSnode.scriptPubKey.begin(), pblock->txoutXFSnode.scriptPubKey.end())));
        xfsnodeObj.push_back(Pair("amount", pblock->txoutXFSnode.nValue));
    }
    result.push_back(Pair("xfsnode", xfsnodeObj));
    result.push_back(Pair("xfsnode_payments_started", pindexPrev->nHeight + 1 > Params().GetConsensus().nXFSnodePaymentsStartBlock));
    result.push_back(Pair("xfsnode_payments_enforced", sporkManager.IsSporkActive(SPORK_8_XFSNODE_PAYMENT_ENFORCEMENT)));
