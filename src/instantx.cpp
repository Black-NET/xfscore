...
// step 2) Top COutPointLock::SIGNATURES_TOTAL xfsnodes per each spent outpoint push "txvote" message
...
//    if(!sporkManager.IsSporkActive(SPORK_2_INSTANTSEND_ENABLED)) return;
    ...
    // Ignore any InstantSend messages until xfsnode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // XFSnodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetXFSnodeRank(activeXFSnode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown XFSnode %s\n", activeXFSnode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- XFSnode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasXFSnodeVoted(itOutpointLock->first, activeXFSnode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeXFSnode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  xfsnode=%s new\n",
                    txHash.ToString(), vote.GetXFSnodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  xfsnode=%s seen\n",
                    txHash.ToString(), vote.GetXFSnodeOutpoint().ToStringShort());
        }
        ...
        int nXFSnodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapXFSnodeOrphanVotes.count(vote.GetXFSnodeOutpoint())) {
            mapXFSnodeOrphanVotes[vote.GetXFSnodeOutpoint()] = nXFSnodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapXFSnodeOrphanVotes[vote.GetXFSnodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageXFSnodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- xfsnode is spamming orphan Transaction Lock Votes: txid=%s  xfsnode=%s\n",
                        txHash.ToString(), vote.GetXFSnodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasXFSnodeVoted(vote.GetOutpoint(), vote.GetXFSnodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same xfsnode.
                    // TODO: apply pose ban score to this xfsnode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- xfsnode sent conflicting votes! %s\n", vote.GetXFSnodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other xfsnodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageXFSnodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapXFSnodeOrphanVotes is empty
    if(mapXFSnodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapXFSnodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapXFSnodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapXFSnodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  xfsnode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetXFSnodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  xfsnode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetXFSnodeOutpoint().ToStringShort());
    ...
    // remove expired xfsnode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itXFSnodeOrphan = mapXFSnodeOrphanVotes.begin();
    while(itXFSnodeOrphan != mapXFSnodeOrphanVotes.end()) {
        if(itXFSnodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan xfsnode vote: xfsnode=%s\n",
                    itXFSnodeOrphan->first.ToStringShort());
            mapXFSnodeOrphanVotes.erase(itXFSnodeOrphan++);
        } else {
            ++itXFSnodeOrphan;
        }
    }
...
{
    if(!mnodeman.Has(CTxIn(outpointXFSnode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown xfsnode %s\n", outpointXFSnode.ToStringShort());
        mnodeman.AskForMN(pnode, CTxIn(outpointXFSnode));
        return false;
    }
    ...
    int n = mnodeman.GetXFSnodeRank(CTxIn(outpointXFSnode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated xfsnode %s\n", outpointXFSnode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- XFSnode %s, rank=%d\n", outpointXFSnode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- XFSnode %s is not in the top %d (%d), vote hash=%s\n",
                outpointXFSnode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    xfsnode_info_t infoMn = mnodeman.GetXFSnodeInfo(CTxIn(outpointXFSnode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown XFSnode: xfsnode=%s\n", outpointXFSnode.ToString());
        return false;
    }

    if(!darkSendSigner.VerifyMessage(infoMn.pubKeyXFSnode, vchXFSnodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!darkSendSigner.SignMessage(strMessage, vchXFSnodeSignature, activeXFSnode.keyXFSnode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!darkSendSigner.VerifyMessage(activeXFSnode.pubKeyXFSnode, vchXFSnodeSignature, strMessage, strError)) {
...
{
    if(mapXFSnodeVotes.count(vote.GetXFSnodeOutpoint()))
        return false;
    mapXFSnodeVotes.insert(std::make_pair(vote.GetXFSnodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapXFSnodeVotes.begin();
    while(itVote != mapXFSnodeVotes.end()) {
...
bool COutPointLock::HasXFSnodeVoted(const COutPoint& outpointXFSnodeIn) const
{
    return mapXFSnodeVotes.count(outpointXFSnodeIn);
}
...
bool CTxLockCandidate::HasXFSnodeVoted(const COutPoint& outpointIn, const COutPoint& outpointXFSnodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasXFSnodeVoted(outpointXFSnodeIn);
}
