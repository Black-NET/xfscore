...
void PrepareShutdown(){
    ...
    CFlatDB<CXFSnodeMan> flatdb1("incache.dat", "magicXFSnodeCache");
    flatdb1.Dump(mnodeman);
    CFlatDB<CXFSnodePayments> flatdb2("inpayments.dat", "magicXFSnodePaymentsCache");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-resync", false)) {
            ...
            fs::path xfsnodeCache = GetDataDir() / "incache.dat";
            fs::path xfsnodePayments = GetDataDir() / "inpayments.dat";
            ...
            try {
                ...
                if (fs::exists(xfsnodeCache)){
                    fs::remove(xfsnodeCache);
                    LogPrintf("-resync: file deleted: %s\n", xfsnodeCache);
                }
                if (fs::exists(xfsnodePayments)){
                    fs::remove(xfsnodePayments);
                    LogPrintf("-resync: file deleted: %s\n", xfsnodePayments);
                }
...
//    nCoinCacheUsage = nTotalCache; // the rest goes to in-memory cache
    ...
    fXFSNode = GetBoolArg("-xfsnode", false);

    LogPrintf("fXFSNode = %s\n", fXFSNode);
    LogPrintf("xfsnodeConfig.getCount(): %s\n", xfsnodeConfig.getCount());

    if ((fXFSNode || xfsnodeConfig.getCount() > 0) && !fTxIndex) {
        return InitError("Enabling XFSnode support requires turning on transaction indexing."
    ...
    if (fXFSNode) {
        ...
        if (!GetArg("-xfsnodeaddr", "").empty()) {
            // Hot XFSnode (either local or remote) should get its address in
            // CActiveXFSnode::ManageState() automatically and no longer relies on XFSnodeaddr.
            return InitError(_("xfsnodeaddr option is deprecated. Please use xfsnode.conf to manage your remote xfsnodes."));
        }

        std::string strXFSnodePrivKey = GetArg("-xfsnodeprivkey", "");
        if (!strXFSnodePrivKey.empty()) {
            if (!darkSendSigner.GetKeysFromSecret(strXFSnodePrivKey, activeXFSnode.keyXFSnode,
                                                  activeXFSnode.pubKeyXFSnode))
                return InitError(_("Invalid xfsnodeprivkey. Please see documentation."));

            LogPrintf("  pubKeyXFSnode: %s\n", CBitcoinAddress(activeXFSnode.pubKeyXFSnode.GetID()).ToString());
        } else {
            return InitError(
                    _("You must specify a xfsnodeprivkey in the configuration. Please see documentation for help."));
        }
    }

    LogPrintf("Using XFSnode config file %s\n", GetXFSnodeConfigFile().string());

    // Lock Existing XFSnodes
    if (GetBoolArg("-inconflock", true) && (xfsnodeConfig.getCount() > 0)) {
        LogPrintf(" Locking Existing XFSnodes..\n");
        ...
        BOOST_FOREACH(CXFSnodeConfig::CXFSnodeEntry mne, xfsnodeConfig.getEntries()) {
            ...
            if(pwalletMain->IsMine(CTxIn(outpoint)) == ISMINE_SPENDABLE &&
              !pwalletMain->IsSpent(mnTxHash, outputIndex)){
                pwalletMain->LockCoin(outpoint); //Lock if this transaction is an available xfsnode colleteral payment
            }else {
                pwalletMain->UnlockCoin(outpoint); // Unlock any spent/unavailable XFSnode collateral
            }
        }
        if(fApi)
...
//    nInstantSendDepth = std::min(std::max(nInstantSendDepth, 0), 60);
    ...
    // lite mode disables all XFSnode and Darksend related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if (fXFSNode && fLiteMode) {
        return InitError("You can not start a xfsnode in litemode");
    }
...
//    LogPrintf("PrivateSend amount %d\n", nPrivateSendAmount);
    ...
    if (GetBoolArg("-persistentxfsnodestate", true)) {
        uiInterface.InitMessage(_("Loading xfsnode cache..."));
        CFlatDB<CXFSnodeMan> flatdb1("incache.dat", "magicXFSnodeCache");
        if (!flatdb1.Load(mnodeman)) {
            return InitError("Failed to load xfsnode cache from incache.dat");
        }

        if (mnodeman.size()) {
            uiInterface.InitMessage(_("Loading XFSnode payment cache..."));
            CFlatDB<CXFSnodePayments> flatdb2("inpayments.dat", "magicXFSnodePaymentsCache");
            if (!flatdb2.Load(mnpayments)) {
                return InitError("Failed to load xfsnode payments cache from inpayments.dat");
            }
        } else {
            uiInterface.InitMessage(_("XFSnode cache is empty, skipping payments cache..."));
        }
    ...
    // ********************************************************* Step 11c: update block tip in XFS modules
