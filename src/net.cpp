...
CNode *ConnectNode(CAddress addrConnect, const char *pszDest, bool fCountFailure, bool fConnectToXFSnode) {
    if (pszDest == NULL) {
        // we clean xfsnode connections in CXFSnodeMan::ProcessXFSnodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveXFSnode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToXFSnode)
            return NULL;
        LOCK(cs_vNodes);
        // Look for an existing connection
        CNode *pnode = FindNode((CService) addrConnect);
        if (pnode) {
            // we have existing connection to this node but it was not a connection to xfsnode,
            // change flag and add reference so that we can correctly clear it later
            if (fConnectToXFSnode && !pnode->fXFSnode) {
                pnode->fXFSnode = true;
            }
            pnode->AddRef();
            return pnode;
