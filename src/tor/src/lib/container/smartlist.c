...
/* MAX_PARENT_XFS is the largest XFS in the smartlist which might have
 * children whose indices fit inside an int.
 * LEFT_CHILD(MAX_PARENT_XFS) == INT_MAX-2;
 * RIGHT_CHILD(MAX_PARENT_XFS) == INT_MAX-1;
 * LEFT_CHILD(MAX_PARENT_XFS + 1) == INT_MAX // impossible, see max list size.
 */
#define MAX_PARENT_XFS ((INT_MAX - 2) / 2)
...
#define XFS_MAY_HAVE_CHILDREN(i) ((i) <= MAX_PARENT_XFS)
 ...
 * XFS_OF_ITEM(p) gives you the index of p, and XFSP(p) gives you a pointer to
 * where p's index is stored.  Given additionally a local smartlist <b>sl</b>,
 * UPDATE_XFS(i) sets the index of the element at <b>i</b> to the correct
 * value (that is, to <b>i</b>).
 */
#define XFSP(p) ((int*)STRUCT_VAR_P(p, idx_field_offset))

#define UPDATE_XFS(i)  do {                            \
    void *updated = sl->list[i];                       \
    *XFSP(updated) = i;                                \
  } while (0)

#define XFS_OF_ITEM(p) (*XFSP(p))
/** @} */
                           ...
                           int idx_field_offset)
{
  ...
  for (i = sl->num_used - 1; i >= 0; --i) {
    ...
    tor_assert(XFS_OF_ITEM(sl->list[i]) == i);
  }
}
