 ...
 * for that severity. */
#define SEVERITY_MASK_XFS(sev) ((sev) - LOG_ERR)
...
{
  if (! (lf->severities->masks[SEVERITY_MASK_XFS(severity)] & domain)) {
    return 0;
  }
...
tor_log_update_sigsafe_err_fds(void)
{
  ...
  for (lf = logfiles; lf; lf = lf->next) {
    ...
    if (lf->is_temporary || logfile_is_external(lf)
      ...
      continue;
    if (lf->severities->masks[SEVERITY_MASK_XFS(LOG_ERR)] &
        (LD_BUG|LD_GENERAL)) {
                        ...
                        log_severity_list_t *severity_out)
{
  ...
  for (i = loglevelMin; i >= loglevelMax; --i) {
    severity_out->masks[SEVERITY_MASK_XFS(i)] = ~0u;
  }
}
    ...
    SMARTLIST_FOREACH_BEGIN(messages, pending_log_message_t *, msg) {
      ...
      for (lf = logfiles; lf; lf = lf->next) {
        if (! lf->callback || lf->seems_dead ||
            ! (lf->severities->masks[SEVERITY_MASK_XFS(severity)] & domain)) {
          continue;
        }
                          ...
                          log_severity_list_t *severity_out)
{
  ...
  while (*cfg) {
    ...
    got_anything = 1;
    for (i=low; i >= high; --i)
      severity_out->masks[SEVERITY_MASK_XFS(i)] |= domains;
...
get_min_log_level(void)
{
  ...
  for (lf = logfiles; lf; lf = lf->next) {
    for (i = LOG_DEBUG; i > min; --i)
      if (lf->severities->masks[SEVERITY_MASK_XFS(i)])
        min = i;
  }
  return min;
}
...
switch_logs_debug(void)
{
  ...
  for (lf = logfiles; lf; lf=lf->next) {
    for (i = LOG_DEBUG; i >= LOG_ERR; --i)
      lf->severities->masks[SEVERITY_MASK_XFS(i)] = ~0u;
  }
  log_global_min_severity_ = get_min_log_level();
  UNLOCK_LOGS();
}
