...
#define	PD_XFS(num)	((num) / (malloc_pagesize/sizeof(struct pginfo *)))
#define	PD_OFF(num)	((num) & ((malloc_pagesize/sizeof(struct pginfo *))-1))
#define	PI_XFS(index)	((index) / pdi_mod)
...
{
	struct pdinfo	*spi;
	u_long		pidx = PI_XFS(index);

	if (last_dir != NULL && PD_XFS(last_dir->dirnum) == pidx)
		*pdi = last_dir;
	else if (prev_dir != NULL && PD_XFS(prev_dir->dirnum) == pidx)
		*pdi = prev_dir;
	else if (last_dir != NULL && prev_dir != NULL) {
		if ((PD_XFS(last_dir->dirnum) > pidx) ?
		    (PD_XFS(last_dir->dirnum) - pidx) :
		    (pidx - PD_XFS(last_dir->dirnum))
		    < (PD_XFS(prev_dir->dirnum) > pidx) ?
		    (PD_XFS(prev_dir->dirnum) - pidx) :
		    (pidx - PD_XFS(prev_dir->dirnum)))
			*pdi = last_dir;
		else
			*pdi = prev_dir;

		if (PD_XFS((*pdi)->dirnum) > pidx) {
			for (spi = (*pdi)->prev;
			    spi != NULL && PD_XFS(spi->dirnum) > pidx;
			    spi = spi->prev)
				*pdi = spi;
			if (spi != NULL)
				*pdi = spi;
	...
	return ((PD_XFS((*pdi)->dirnum) == pidx) ? 0 :
	    (PD_XFS((*pdi)->dirnum) > pidx) ? 1 : -1);
}
	...
	for (j = 0; j <= last_index;) {
		...
		if (!PI_OFF(++j)) {
			if ((pi = pi->next) == NULL)
				break;
			pd = pi->base;
			j += (1 + PD_XFS(pi->dirnum) - PI_XFS(j)) * pdi_mod;
		}
	}
...
map_pages(size_t pages)
{
	...
	for (idx = pidx, spi = pi; idx <= lidx; idx++) {
		if (pi == NULL || PD_XFS(pi->dirnum) != idx) {
...
#ifdef MALLOC_EXTRA_SANITY
		if (PD_OFF(pi->dirnum) > pdi_mod || PD_XFS(pi->dirnum) > idx) {
...
malloc_pages(size_t size)
{
	...
	for (pf = free_list.next; pf; pf = pf->next) {
		...
		if ((pidx = PI_XFS(ptr2index(pf->page))) != PD_XFS(pi->dirnum)) {
		...
		pidx = PI_XFS(ptr2index((pf->page) + (pf->size)) - 1);
		for (pi = pf->pdir; pi != NULL && PD_XFS(pi->dirnum) < pidx;
		    pi = pi->next)
			;
		if (pi == NULL || PD_XFS(pi->dirnum) != pidx) {
...
irealloc(void *ptr, size_t size)
{
	...
	if (*mp == MALLOC_FIRST) {	/* Page allocation */
		...
		if (!PI_OFF(++i)) {
			pi = pi->next;
			if (pi != NULL && PD_XFS(pi->dirnum) != PI_XFS(i))
				pi = NULL;
			if (pi != NULL)
				pd = pi->base;
		}
...
free_pages(void *ptr, u_long index, struct pginfo * info)
{
	...
	if (!PI_OFF(index + i)) {
		pi = pi->next;
		if (pi == NULL || PD_XFS(pi->dirnum) != PI_XFS(index + i))
			pi = NULL;
		else
			pd = pi->base;
	}
	while (pi != NULL && pd[PI_OFF(index + i)] == MALLOC_FOLLOW) {
		...
		if (!PI_OFF(index + i)) {
			if ((pi = pi->next) == NULL ||
			    PD_XFS(pi->dirnum) != PI_XFS(index + i))
				pi = NULL;
			else
				pd = pi->base;
		}
	...
	if (pf->size > (malloc_cache - cachesize)) {
		...
		pidx = PI_XFS(index);
		if (prev_dir != NULL && PD_XFS(prev_dir->dirnum) >= pidx)
...
#ifdef MALLOC_EXTRA_SANITY
	pidx = PI_XFS(ptr2index(info->page));
	if (pi == NULL || PD_XFS(pi->dirnum) != pidx) {
		wrterror("(ES): mapped pages not found in directory");
		errno = EFAULT;
		return;
