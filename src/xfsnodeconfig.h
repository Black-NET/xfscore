...
#ifndef SRC_XFSNODECONFIG_H_
#define SRC_XFSNODECONFIG_H_

#include "fs.h"

#include <univalue.h>

class CXFSnodeConfig;
extern CXFSnodeConfig xfsnodeConfig;
...
public:
    ...
    public:
        ...
        CXFSnodeEntry(std::string alias, std::string ip, std::string privKey, std::string txHash, std::string outputIndex) {
            this->alias = alias;
            this->ip = ip;
    ...
    CXFSnodeConfig() {
        entries = std::vector<CXFSnodeEntry>();
    }
    ...
    std::vector<CXFSnodeEntry>& getEntries() {
        return entries;
    }
...
private:
    std::vector<CXFSnodeEntry> entries;


};


#endif /* SRC_XFSNODECONFIG_H_ */
