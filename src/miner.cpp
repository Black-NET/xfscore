...
//            }
        ...
        // Update coinbase transaction with additional info about xfsnode and governance payments,
        // get some info back to pass to getblocktemplate
        bool fPayXFSNode = nHeight >= chainparams.GetConsensus().nXFSnodePaymentsStartBlock && !fProofOfStake;
        CAmount xfsnodePayment = 0;
        if (fPayXFSNode) {
            const Consensus::Params &params = chainparams.GetConsensus();
            xfsnodePayment = GetXFSnodePayment(chainparams.GetConsensus(),false,nHeight);
            FillBlockPayments(coinbaseTx, nHeight, xfsnodePayment, pblock->txoutXFSnode, pblock->voutSuperblock);
        }
        //Only take out idx payment if a xfsnode is actually filled in txoutxfsnode and xfsnodepayment is not 0
        if(pblock->txoutXFSnode != CTxOut() && xfsnodePayment != 0)
            coinbaseTx.vout[0].nValue -= xfsnodePayment;
    ...
    try {
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                // Also try to wait for xfsnode winners unless we're on regtest chain
                do {
                    bool fvNodesEmpty;
                    bool fHasXFSnodesWinnerForNextBlock;
                    const Consensus::Params &params = chainparams.GetConsensus();
                    {
                        LOCK(cs_vNodes);
                    ...
                    }
                    {
                        LOCK2(cs_main, mempool.cs);
                        int nCount = 0;
                        fHasXFSnodesWinnerForNextBlock =
                                params.IsRegtest() ||
                                chainActive.Height() < params.nXFSnodePaymentsStartBlock ||
                                mnodeman.GetNextXFSnodeInQueueForPayment(chainActive.Height(), true, nCount);
                    }
                    if (!fvNodesEmpty && fHasXFSnodesWinnerForNextBlock && !IsInitialBlockDownload()) {
                        break;
                    }
                    MilliSleep(1000);
                } while (true);
...
void ThreadStakeMiner(CWallet *pwallet, const CChainParams& chainparams)
{
    ...
    while (true)
    {
        ...
        if (nHeight >= Params().GetConsensus().nFirstPOSBlock)
        {
            ...
            while (vNodes.empty() || IsInitialBlockDownload() || !xfsnodeSync.IsSynced())
            {
                nLastCoinStakeSearchInterval = 0;
                fTryToSync = true;
                MilliSleep(1000);
