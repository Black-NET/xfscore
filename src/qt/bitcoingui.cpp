...
{
    GUIUtil::restoreWindowGeometry("nWindow", QSize(900, 600), this);
...
void BitcoinGUI::createActions()
{
	...
	sendCoinsAction->setStatusTip(tr("Send coins to a XFS address"));
    ...
    xfsnodeAction = new QAction(platformStyle->MultiStatesIcon(":/icons/xfsnodes"), tr("&XFSnodes"), this);
    xfsnodeAction->setStatusTip(tr("Browse xfsnodes"));
    xfsnodeAction->setToolTip(xfsnodeAction->statusTip());
    xfsnodeAction->setCheckable(true);
#ifdef Q_OS_MAC
    xfsnodeAction->setShortcut(QKeySequence(Qt::CTRL + key++));
#else
    xfsnodeAction->setShortcut(QKeySequence(Qt::ALT +  key++));
#endif
    tabGroup->addAction(xfsnodeAction);
#endif
...
#ifdef ENABLE_WALLET
    connect(xfsnodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(xfsnodeAction, SIGNAL(triggered()), this, SLOT(gotoXFSnodePage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your XFS addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->SingleColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified XFS addresses"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible XFS command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(xfsnodeAction);
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    xfsnodeAction->setEnabled(enabled);
...
void BitcoinGUI::gotoXFSnodePage()
{
    QSettings settings;
    xfsnodeAction->setChecked(true);
    if (walletFrame) walletFrame->gotoXFSnodePage();
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to XFS network", "", count));
}
...
void BitcoinGUI::setAdditionalDataSyncProgress(int count, double nSyncProgress)
{
    ...
    if(xfsnodeSync.GetBlockchainSynced())
    {
        ...
        strSyncStatus = QString(xfsnodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("XFS"); // default title
