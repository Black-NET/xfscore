...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. XFS can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse xfs.conf
...
#ifdef ENABLE_WALLET
    /// 7a. parse xfsnode.conf
    std::string strErr;
    if(!xfsnodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("XFS Core"),
                              QObject::tr("Error reading xfsnode configuration file: %1").arg(strErr.c_str()));
        return EXIT_FAILURE;
    }
