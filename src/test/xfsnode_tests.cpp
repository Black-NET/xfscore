...
CScript scriptPubKeyXFSnode;


struct XFSnodeTestingSetup : public TestingSetup {
    XFSnodeTestingSetup() : TestingSetup(CBaseChainParams::REGTEST)
    {
        CPubKey newKey;
        ...
        scriptPubKeyXFSnode = CScript() <<  ToByteVector(newKey/*coinbaseKey.GetPubKey()*/) << OP_CHECKSIG;
        bool mtp = false;
        CBlock b;
        for (int i = 0; i < 150; i++)
        {
            std::vector<CMutableTransaction> noTxns;
            b = CreateAndProcessBlock(noTxns, scriptPubKeyXFSnode, mtp);
    ...
    CBlock CreateBlock(const std::vector<CMutableTransaction>& txns,
                       const CScript& scriptPubKeyXFSnode, bool mtp = false) {
    ...
    // scriptPubKeyXFSnode, and try to add it to the current chain.
    CBlock CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns,
                                 const CScript& scriptPubKeyXFSnode, bool mtp = false){

        CBlock block = CreateBlock(txns, scriptPubKeyXFSnode, mtp);
...
BOOST_FIXTURE_TEST_SUITE(xfsnode_tests, XFSnodeTestingSetup)

BOOST_AUTO_TEST_CASE(Test_EnforceXFSnodePayment)
{

    std::vector<CMutableTransaction> noTxns;
    CBlock b = CreateAndProcessBlock(noTxns, scriptPubKeyXFSnode, false);
    const CChainParams& chainparams = Params();
    ...
    auto const before_block = ZC_XFSNODE_PAYMENT_BUG_FIXED_AT_BLOCK
             , after_block = ZC_XFSNODE_PAYMENT_BUG_FIXED_AT_BLOCK + 1;
    // Emulates synced state of xfsnodes.
    for(size_t i =0; i < 4; ++i)
        xfsnodeSync.SwitchToNextAsset();


    ///////////////////////////////////////////////////////////////////////////
    // Paying to the best payee
    CXFSnodePayee payee1(tx.vout[1].scriptPubKey, uint256());
    // Emulates 6 votes for the payee
    for(size_t i =0; i < 5; ++i)
        payee1.AddVoteHash(uint256());

    CXFSnodeBlockPayees payees;
    payees.vecPayees.push_back(payee1);

    mnpayments.mapXFSnodeBlocks[after_block] = payees;
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Making xfsnodes not synchronized and checking the functionality is disabled
    xfsnodeSync.Reset();
    b.fChecked = false;
    b.hashMerkleRoot = BlockMerkleRoot(b, &mutated);
    while (!CheckProofOfWork(b.GetPoWHash(), b.nBits, chainparams.GetConsensus())){
    ...
    CXFSnodePayee payee2(tx.vout[0].scriptPubKey, uint256());
    // Emulates 9 votes for the payee
    for(size_t i =0; i < 8; ++i)
        payee2.AddVoteHash(uint256());

    mnpayments.mapXFSnodeBlocks[after_block].vecPayees.insert(mnpayments.mapXFSnodeBlocks[after_block].vecPayees.begin(), payee2);
    ...
    mnpayments.mapXFSnodeBlocks[before_block] = payees;
