...
BOOST_AUTO_TEST_CASE(xfsnodelist_test)
{
    // Verify "Create" initially.
    UniValue valRequest(UniValue::VOBJ);
    UniValue data(UniValue::VOBJ);
    ...
    valRequest.push_back(Pair("type", "initial"));
    valRequest.push_back(Pair("collection", "xfsnodeList"));
    //valRequest.push_back(Pair("auth", auth));

    BOOST_CHECK_THROW(CallAPI(valRequest, true), runtime_error);

    // artificially finish xfsnode sync to test list call
    while(!xfsnodeSync.IsSynced()){
        xfsnodeSync.SwitchToNextAsset();
    }

    result = CallAPI(valRequest, true);
    BOOST_CHECK(!result.isNull()); // empty xfsnode list
}
