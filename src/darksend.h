...
// A helper object for signing messages from XFSnodes
...
public:
     ...
     *     1) we have an active XFSnode,
     *     2) we have a valid XFSnode private key,
     *     3) we signed the message successfully, and
     *     4) we verified the message successfully
     */
    bool Sign();
    /// Check if we have a valid XFSnode address
    bool CheckSignature(const CPubKey& pubKeyXFSnode);
    ...
    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, xfsnode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 5000 XFS - checking if valid xfsnode)
...
private:
    ...
    // Keep track of the used XFSnodes
    std::vector<CTxIn> vecXFSnodesUsed;
    ...
    std::vector<CDarkSendEntry> vecEntries; // XFSnode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a XFSnode to start the process
    ...
    /// Get XFSnode updates about the progress of mixing
