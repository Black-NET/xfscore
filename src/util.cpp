...
const char * const BITCOIN_CONF_FILENAME = "xfs.conf";
const char * const BITCOIN_PID_FILENAME = "xfsd.pid";
...
fs::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\xfsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\xfs
    // Mac: ~/Library/Application Support/xfs
    // Unix: ~/.xfs
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "xfs";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/xfs";
#else
    // Unix
    return pathRet / ".xfs";
#endif
...
fs::path GetXFSnodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-znconf", "xfsnode.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override xfs.conf
