...
void CDarksendPool::ProcessMessage(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    if (fLiteMode) return; // ignore all XFS related functionality
    ...
    if (strCommand == NetMsgType::DSACCEPT) {
        ...
        if (!fXFSNode) {
            LogPrintf("DSACCEPT -- not a XFSnode!\n");
        ...
        CXFSnode *pmn = mnodeman.Find(activeXFSnode.vin);
        if (pmn == NULL) {
        ...
        CXFSnode *pmn = mnodeman.Find(dsq.vin);
        if (pmn == NULL) return;

        if (!dsq.CheckSignature(pmn->pubKeyXFSnode)) {
        ...
        if (dsq.fReady) {
            if (!pSubmittedToXFSnode) return;
            if ((CNetAddr) pSubmittedToXFSnode->addr != (CNetAddr) pmn->addr) {
                LogPrintf("DSQUEUE -- message doesn't match current XFSnode: pSubmittedToXFSnode=%s, addr=%s\n", pSubmittedToXFSnode->addr.ToString(), pmn->addr.ToString());
                return;
            }

            if (nState == POOL_STATE_QUEUE) {
                LogPrint("privatesend", "DSQUEUE -- PrivateSend queue (%s) is ready on xfsnode %s\n", dsq.ToString(), pmn->addr.ToString());
                SubmitDenominate();
            }
        } else {
            BOOST_FOREACH(CDarksendQueue
            q, vecDarksendQueue) {
                if (q.vin == dsq.vin) {
                    ...
                    LogPrint("privatesend", "DSQUEUE -- XFSnode %s is sending WAY too many dsq messages\n", pmn->addr.ToString());
                    return;
                }
            }
            ...
            if (pmn->nLastDsq != 0 && nThreshold > mnodeman.nDsqCount) {
                LogPrint("privatesend", "DSQUEUE -- XFSnode %s is sending too many dsq messages\n", pmn->addr.ToString());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("privatesend", "DSQUEUE -- new PrivateSend queue (%s) from xfsnode %s\n", dsq.ToString(), pmn->addr.ToString());
            if (pSubmittedToXFSnode && pSubmittedToXFSnode->vin.prevout == dsq.vin.prevout) {
                dsq.fTried = true;
            }
        ...
        if (fXFSNode) {
            // LogPrintf("DSSTATUSUPDATE -- Can't run on a XFSnode!\n");
            return;
        }

        if (!pSubmittedToXFSnode) return;
        if ((CNetAddr) pSubmittedToXFSnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSSTATUSUPDATE -- message doesn't match current XFSnode: pSubmittedToXFSnode %s addr %s\n", pSubmittedToXFSnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (!fXFSNode) {
            LogPrintf("DSSIGNFINALTX -- not a XFSnode!\n");
            return;
        }
        ...
        if (fXFSNode) {
            // LogPrintf("DSFINALTX -- Can't run on a XFSnode!\n");
            return;
        }

        if (!pSubmittedToXFSnode) return;
        if ((CNetAddr) pSubmittedToXFSnode->addr != (CNetAddr) pfrom->addr) {
            //LogPrintf("DSFINALTX -- message doesn't match current XFSnode: pSubmittedToXFSnode %s addr %s\n", pSubmittedToXFSnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
        ...
        if (fXFSNode) {
            // LogPrintf("DSCOMPLETE -- Can't run on a XFSnode!\n");
            return;
        }

        if (!pSubmittedToXFSnode) return;
        if ((CNetAddr) pSubmittedToXFSnode->addr != (CNetAddr) pfrom->addr) {
            LogPrint("privatesend", "DSCOMPLETE -- message doesn't match current XFSnode: pSubmittedToXFSnode=%s  addr=%s\n", pSubmittedToXFSnode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
...
std::string CDarksendPool::GetStatus() {
    ...
    if ((pCurrentBlockIndex && pCurrentBlockIndex->nHeight - nCachedLastSuccessBlock < nMinBlockSpacing) || !xfsnodeSync.GetBlockchainSynced())
    ...
    switch (nState) {
        ...
        case POOL_STATE_QUEUE:
            ...
            return strprintf(_("Submitted to xfsnode, waiting in queue %s"), strSuffix);;
        case POOL_STATE_ACCEPTING_ENTRIES:
            if (nEntriesCount == 0) {
                ...
                if (nStatusMessageProgress % 70 <= 40) return strprintf(_("Submitted following entries to xfsnode: %u / %d"), nEntriesCount, GetMaxPoolTransactions());
                else if (nStatusMessageProgress % 70 <= 50) strSuffix = ".";
                else if (nStatusMessageProgress % 70 <= 60) strSuffix = "..";
                else if (nStatusMessageProgress % 70 <= 70) strSuffix = "...";
                return strprintf(_("Submitted to xfsnode, waiting for more entries ( %u / %d ) %s"), nEntriesCount, GetMaxPoolTransactions(), strSuffix);
            }
        case POOL_STATE_SIGNING:
...
//
// Check the mixing progress and send client updates if a XFSnode
//
...
void CDarksendPool::CommitFinalTransaction() {
    if (!fXFSNode) return; // check and relay final tx only on xfsnode
    ...
    // create and sign xfsnode dstx transaction
    if (!mapDarksendBroadcastTxes.count(hashTx)) {
        CDarksendBroadcastTx dstx(finalTransaction, activeXFSnode.vin, GetAdjustedTime());
        dstx.Sign();
...
//
// How does this work? Messages to XFSnodes come in via NetMsgType::DSVIN, these require a valid collateral
// transaction for the client to be able to enter the pool. This transaction is kept by the XFSnode
...
/*
    ...
    allow endless transaction that would bloat XFS and make it unusable. To
...
void CDarksendPool::CheckTimeout() {
    ...
    if (!fEnablePrivateSend && !fXFSNode) return;
    ...
    int nLagTime = fXFSNode ? 0 : 10000; // if we're the client, give the server a few extra seconds before resetting.
...
void CDarksendPool::CheckForCompleteQueue() {
    ...
    if (nState == POOL_STATE_QUEUE && IsSessionReady()) {
        ...
        CDarksendQueue dsq(nSessionDenom, activeXFSnode.vin, GetTime(), true);
...
//
// Execute a mixing denomination via a XFSnode.
// This is only ran from clients
//
bool CDarksendPool::SendDenominate(const std::vector <CTxIn> &vecTxIn, const std::vector <CTxOut> &vecTxOut) {
    if (fXFSNode) {
        LogPrintf("CDarksendPool::SendDenominate -- PrivateSend from a XFSnode is not supported currently.\n");
        return false;
    }
    ...
    // we should already be connected to a XFSnode
    if (!nSessionID) {
        LogPrintf("CDarksendPool::SendDenominate -- No XFSnode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return false;
    }
...
// Incoming message from XFSnode updating the progress of mixing
bool CDarksendPool::CheckPoolStateUpdate(PoolState nStateNew, int nEntriesCountNew, PoolStatusUpdate nStatusUpdate, PoolMessage nMessageID, int nSessionIDNew) {
    ...
    strAutoDenomResult = _("XFSnode:") + " " + GetMessageByID(nMessageID);

    // if rejected at any state
    if (nStatusUpdate == STATUS_REJECTED) {
        LogPrintf("CDarksendPool::CheckPoolStateUpdate -- entry is rejected by XFSnode\n");
        UnlockCoins();
        SetNull();
        SetState(POOL_STATE_ERROR);
...
bool CDarksendPool::SignFinalTransaction(const CTransaction &finalTransactionNew, CNode *pnode) {
    ...
    // push all of our signatures to the XFSnode
    LogPrintf("CDarksendPool::SignFinalTransaction -- pushing sigs to the xfsnode, finalMutableTransaction=%s", finalMutableTransaction.ToString());
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun) {
    if (!fEnablePrivateSend || fXFSNode || !pCurrentBlockIndex) return false;
    ...
    if (mnodeman.size() == 0) {
        LogPrint("privatesend", "CDarksendPool::DoAutomaticDenominating -- No XFSnodes detected\n");
        strAutoDenomResult = _("No XFSnodes detected.");
        return false;
    }
    ...
    // If we've used 90% of the XFSnode list then drop the oldest first ~30%
    int nThreshold_high = nMnCountEnabled * 0.9;
    int nThreshold_low = nThreshold_high * 0.7;
    LogPrint("privatesend", "Checking vecXFSnodesUsed: size: %d, threshold: %d\n", (int) vecXFSnodesUsed.size(), nThreshold_high);

    if ((int) vecXFSnodesUsed.size() > nThreshold_high) {
        vecXFSnodesUsed.erase(vecXFSnodesUsed.begin(), vecXFSnodesUsed.begin() + vecXFSnodesUsed.size() - nThreshold_low);
        LogPrint("privatesend", "  vecXFSnodesUsed: new size: %d, threshold: %d\n", (int) vecXFSnodesUsed.size(), nThreshold_high);
    }
    ...
    if (nLiquidityProvider || fUseQueue) {
        ...
        BOOST_FOREACH(CDarksendQueue & dsq, vecDarksendQueue)
        {
            ...
            CXFSnode *pmn = mnodeman.Find(dsq.vin);
            if (pmn == NULL) {
                LogPrintf("CDarksendPool::DoAutomaticDenominating -- dsq xfsnode is not in xfsnode list, xfsnode=%s\n", dsq.vin.prevout.ToStringShort());
                continue;
            }
            ...
            vecXFSnodesUsed.push_back(dsq.vin);
            ...
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt to connect to xfsnode from queue, addr=%s\n", pmn->addr.ToString());
            // connect to XFSnode and submit the queue request
            CNode *pnode = (pnodeFound && pnodeFound->fXFSnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
            if (pnode) {
                pSubmittedToXFSnode = pmn;
                ...
                strAutoDenomResult = _("Error connecting to XFSnode.");
                continue;
            }
        }
    }
    ...
    while (nTries < 10) {
        CXFSnode *pmn = mnodeman.FindRandomNotInVec(vecXFSnodesUsed, MIN_PRIVATESEND_PEER_PROTO_VERSION);
        if (pmn == NULL) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Can't find random xfsnode!\n");
            strAutoDenomResult = _("Can't find random XFSnode.");
            return false;
        }
        vecXFSnodesUsed.push_back(pmn->vin);

        if (pmn->nLastDsq != 0 && pmn->nLastDsq + nMnCountEnabled / 5 > mnodeman.nDsqCount) {
            LogPrintf("CDarksendPool::DoAutomaticDenominating -- Too early to mix on this xfsnode!"
                              " xfsnode=%s  addr=%s  nLastDsq=%d  CountEnabled/5=%d  nDsqCount=%d\n",
        ...
        LogPrintf("CDarksendPool::DoAutomaticDenominating -- attempt %d connection to XFSnode %s\n", nTries, pmn->addr.ToString());
        CNode *pnode = (pnodeFound && pnodeFound->fXFSnode) ? pnodeFound : ConnectNode(CAddress(pmn->addr, NODE_NETWORK), NULL, false, true);
        if (pnode) {
    ...
    strAutoDenomResult = _("No compatible XFSnode found.");
    return false;
}
...
bool CDarksendPool::CreateNewSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fXFSNode || nSessionID != 0) return false;
    ...
    if (!fUnitTest) {
        ...
        CDarksendQueue dsq(nDenom, activeXFSnode.vin, GetTime(), false);
...
bool CDarksendPool::AddUserToExistingSession(int nDenom, CTransaction txCollateral, PoolMessage &nMessageIDRet) {
    if (!fXFSNode || nSessionID == 0 || IsSessionReady()) return false;
...
std::string CDarksendPool::GetMessageByID(PoolMessage nMessageID) {
    switch (nMessageID) {
        ...
        case ERR_MN_LIST:
            return _("Not in the XFSnode list.");
        case ERR_MODE:
            return _("Incompatible mode.");
        case ERR_NON_STANDARD_PUBKEY:
            return _("Non-standard public key detected.");
        case ERR_NOT_A_MN:
            return _("This is not a XFSnode.");
        case ERR_QUEUE_FULL:
            return _("XFSnode queue is full.");
        case ERR_RECENT:
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn &txin, const CPubKey &pubkey) {
    ...
    if (GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == XFSNODE_COIN_REQUIRED * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CDarksendQueue::Sign() {
    ...
    return CheckSignature(activeXFSnode.pubKeyXFSnode);
}

bool CDarksendQueue::CheckSignature(const CPubKey &pubKeyXFSnode) {
    std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(nTime) + boost::lexical_cast<std::string>(fReady);
    std::string strError = "";

    if (!darkSendSigner.VerifyMessage(pubKeyXFSnode, vchSig, strMessage, strError)) {
        LogPrintf("CDarksendQueue::CheckSignature -- Got bad XFSnode queue signature: %s; error: %s\n", ToString(), strError);
        return false;
    }
...
bool CDarksendBroadcastTx::CheckSignature(const CPubKey &pubKeyXFSnode) {
    std::string strMessage = tx.GetHash().ToString() + boost::lexical_cast<std::string>(sigTime);
    std::string strError = "";
...
void CDarksendPool::RelayIn(const CDarkSendEntry &entry) {
    ...
    CNode *pnode = FindNode(pSubmittedToXFSnode->addr);
    if (pnode != NULL) {
...
void CDarksendPool::SetState(PoolState nStateNew) {
    if (fXFSNode && (nStateNew == POOL_STATE_ERROR || nStateNew == POOL_STATE_SUCCESS)) {
        LogPrint("privatesend", "CDarksendPool::SetState -- Can't set state to ERROR or SUCCESS as a XFSnode. \n");
        return;
    }
...
void CDarksendPool::UpdatedBlockTip(const CBlockIndex *pindex) {
    ...
    if (!fLiteMode && xfsnodeSync.IsXFSnodeListSynced()) {
        NewBlock();
    }
}
...
void ThreadCheckDarkSendPool() {
    ...
    while (true) {
        ...
        xfsnodeSync.ProcessTick();

        if (xfsnodeSync.GetBlockchainSynced() && !ShutdownRequested()) {

            nTick++;

            // make sure to check all xfsnodes first
            mnodeman.Check();

            // check if we should activate or ping every few minutes,
            // slightly postpone first run to give net thread a chance to connect to some peers
            if (nTick % XFSNODE_MIN_MNP_SECONDS == 15)
                activeXFSnode.ManageState();

            if (nTick % 60 == 0) {
                mnodeman.ProcessXFSnodeConnections();
                mnodeman.CheckAndRemove();
                mnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
                GetMainSignals().NotifyXFSnodeList();
            }
            if (fXFSNode && (nTick % (60 * 5) == 0)) {
