...
CPubKey CWallet::GetKeyFromKeypath(uint32_t nChange, uint32_t nChild) {
    ...
    CExtKey coinTypeKey;           //key at m/44'/<1/136>' (Testnet or XFS Coin Type respectively, according to SLIP-0044)
...
bool CWallet::SelectCoinsForStaking(CAmount& nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if(pcoin->IsSigmaMint())
            continue;
        if (n == XFSNODE_COIN_REQUIRED * COIN)
            continue;
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nTime, int64_t nSearchInterval, CAmount& nFees, CMutableTransaction& tx, CKey& key, CBlockTemplate *pblocktemplate)
{
    ...
    // if (GetBoolArg("-stakecache", xfsnodeSync.IsBlockchainSynced())) {
    ...
    // Calculate reward
    {
        ...
        CAmount xfsnodePayment = 0; 
        // xfsnode payments
        if (nHeight >= Params().GetConsensus().nXFSnodePaymentsStartBlock) {
            const CChainParams &chainparams = Params();
            const Consensus::Params &params = chainparams.GetConsensus();
             xfsnodePayment = GetXFSnodePayment(chainparams.GetConsensus(),false,nHeight);
            FillBlockPayments(txNew, nHeight, xfsnodePayment, pblock->txoutXFSnode, pblock->voutSuperblock);
        }
        if(pblock->txoutXFSnode != CTxOut() && xfsnodePayment != 0){
            nReward -= xfsnodePayment;
        }
...
//        }
    ...
    // If XFSnode payment, lock corresponding outpoint
    if (GetBoolArg("-inconflock", true) && (xfsnodeConfig.getCount() > 0)) {
        BOOST_FOREACH(CXFSnodeConfig::CXFSnodeEntry mne, xfsnodeConfig.getEntries()) {
            ...
            if(IsMine(CTxIn(outpoint)) == ISMINE_SPENDABLE && !IsSpent(mnTxHash, outputIndex)){
                LockCoin(outpoint); //Lock if this transaction is an available xfsnode colleteral payment  
            }else {
                UnlockCoin(outpoint); // Unlock any spent XFSnode collateral
            }
        }
    }
...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(nCoinType == ALL_COINS){
                    ...
                    found = !(fXFSNode && pcoin->vout[i].nValue == XFSNODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fXFSNode) found = pcoin->vout[i].nValue != XFSNODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_1000) {
                    found = pcoin->vout[i].nValue == XFSNODE_COIN_REQUIRED * COIN;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fXFSNode && out.tx->vout[out.i].nValue == XFSNODE_COIN_REQUIRED * COIN) continue; //xfsnode input
...
bool CWallet::GetXFSnodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
    ...
    if (vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetXFSnodeVinAndKeys -- Could not locate any valid xfsnode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetXFSnodeVinAndKeys -- Could not locate specified xfsnode vin\n");
    return false;
}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        // xfsnode-like input should not be selected by AvailableCoins now anyway
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fXFSNode && wtx.vout[i].nValue == XFSNODE_COIN_REQUIRED * COIN) continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 5000 XFS.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 5000 XFS.");
                    } else if (nCoinType == ONLY_DENOMINATED) {
...
bool CWallet::CreateZerocoinToSigmaRemintModel(string &stringError, int version, libzerocoin::CoinDenomination denomination, CWalletTx *wtx) {
    ...
    if (!params.IsRegtest() && !xfsnodeSync.IsBlockchainSynced()) {
...
//                }
                    ...
                    //                bool fAllowFree = false;                                 // No free TXs in XFS
    ...
    CMutableTransaction txNew;
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if(thirdPartyaddress == ""){
                ...
                if (!address.IsValid()){
                    strFailReason = _("Invalid XFS address");
                    return false;
                }
                // Parse XFS address
