...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if(chain.nVersion == chain.VERSION_WITH_BIP39){
        throw JSONRPCError(RPC_WALLET_ERROR, "Importing wallets and private keys is disabled for mnemonic-enabled wallets."
                                             "To import your dump file, create a non-mnemonic wallet by setting \"usemnemonic=0\" in your xfs.conf file, after backing up and removing your existing wallet.");
    }
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid XFS address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"indexaddress\"   (string, required) The XFS address for the private key\n"
            "\nResult:\n"
...
UniValue dumpprivkey_index(const UniValue& params, bool fHelp)
{
    ...
    if(params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + params[0].get_str(), params[1].get_str()))
    {
        std::string warning =
            ...
            "XFS team members will never ask for this command's output and it is not needed for XFSnode setup or diagnosis!\n"
            "\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by XFS %s\n", CLIENT_BUILD);
