# Overview
API for interaction with the new `xfs-client` application. This project closely resembles the `rpc` layout, however it instead uses ZeroMQ as a transport mechanism, the code for which is contained within `src/zmqserver`. 
...
| [xfsnodeControl](#xfsnodecontrol)     | Start/stop XFSnode(s) by alias. | 🔐 | ✅ | – |
| [xfsnodeKey](#xfsnodekey)             | Generate a new xfsnode key. | 🔐 | - | – |
| [xfsnodeList](#xfsnodelist)           | list information related to all XFSnodes. | 🔐 | – | – |
                ...
                txids: 
                    {
                        STRING: (txid)
                            { 
                            ["mined"|"send"|"receive"|"xfsnode"|"spend"|"mint"]: (category) 
                                 {
                                    address: STRING,
                                    category: STRING("mined"|"send"|"receive"|"xfsnode"|"spend"|"mint"),
                                    amount: INT,
                                    fee: INT(sats),
                                    label: STRING (VAR : address is part of xfsd "account")
                ...
                total: 
                    {
                        ...
                        balance: INT, (VAR: category=="mined"|"xfsnode"|"receive"|)
                    } 
            },
        [STRING | "MINT"]: (address)
            { 
...
|               | _Event_       | NotifyAPIStatus  | SyncTransaction | NumConnectionsChanged | UpdatedBlockTip | UpdatedMintStatus  | UpdatedSettings | UpdatedXFSnode | UpdateSyncStatus |
| ------------- | ------------- | ---------------  | --------------- | --------------------- | --------------- | -----------------  | --------------- | ------------ | ---------------- |
| **_Topic_**   | Description   | API status notification | new transactions | xfsd peer list updated | blockchain head updated | mint transaction added/up dated | settings changed/updated | XFSnode update | Blockchain sync update
...
**xfsnode** (triggers [xfsnodeUpdate](#xfsnodeupdate))       | update to xfsnode                           | -  | -  | -  | -  | -  | -  | ✅ | -  |
