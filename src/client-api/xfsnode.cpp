...
bool GetXFSnodePayeeAddress(const std::string& txHash, const std::string& n, CBitcoinAddress& address){
...
UniValue xfsnodekey(Type type, const UniValue& data, const UniValue& auth, bool fHelp){
...
UniValue xfsnodecontrol(Type type, const UniValue& data, const UniValue& auth, bool fHelp){
    ...
    switch(type){
        case Update: {
            ...
            if (method == "start-alias") {
                ...
                if ((method == "start-missing") && !xfsnodeSync.IsXFSnodeListSynced()) {
...
UniValue xfsnodelist(Type type, const UniValue& data, const UniValue& auth, bool fHelp){
    ...
    switch(type){
        case Initial: {
            ...
            BOOST_FOREACH(CXFSnodeConfig::CXFSnodeEntry mne, xfsnodeConfig.getEntries()) {
                ...
                CXFSnode* mn = mnodeman.Find(txHash, outputIndex);

                UniValue node(UniValue::VOBJ);
                if(mn==NULL){
                    node = mne.ToJSON();
                    node.push_back(Pair("position", fIndex++));
                    if(GetXFSnodePayeeAddress(txHash, outputIndex, address))
            ...
            /*
             * If the XFSnode list is not yet synced, return the wallet XFSnodes, as described in xfsnode.conf
             * if it is, process all XFSnodes, and return along with wallet XFSnodes.
             * (if the wallet XFSnode has started, it will be replaced in the synced list).
             */
            if(!xfsnodeSync.IsSynced()){
                data.push_back(Pair("nodes", nodes));
                data.push_back(Pair("total", mnodeman.CountXFSnodes()));
                return data;
            }
            ...
            BOOST_FOREACH(CXFSnode & mn, vXFSnodes) {
                ...
                // only process wallet XFSnodes - they are already in "nodes", so if we find it, replace with update
...
UniValue xfsnodeupdate(Type type, const UniValue& data, const UniValue& auth, bool fHelp){
    UniValue ret(UniValue::VOBJ);
...
{ //  category              collection         actor (function)          authPort   authPassphrase   warmupOk
  //  --------------------- ------------       ----------------          -------- --------------   --------
    { "xfsnode",              "xfsnodeControl",    &xfsnodecontrol,            true,      true,            false  },
    { "xfsnode",              "xfsnodeKey",        &xfsnodekey,                true,      false,           false  },
    { "xfsnode",              "xfsnodeList",       &xfsnodelist,               true,      false,           false  },
    { "xfsnode",              "xfsnodeUpdate",     &xfsnodeupdate,             true,      false,           false  }
};
void RegisterXFSnodeAPICommands(CAPITable &tableAPI)
{
