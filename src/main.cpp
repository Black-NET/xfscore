...
#if defined(NDEBUG)
# error "XFS cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "XFS Signed Message:\n";
...
//                                   "0x00000000000743f190a18c5577a3c2d2a1f610ae9601ac046a38084ccb7cd721")));
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (txIds.count(txHash) > 0 && (fTestNet || pindex->nHeight >= HF_XFSNODE_HEIGHT))
    ...
    // XFSNODE : MODIFIED TO CHECK XFSNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (block.nTime > sporkManager.GetSporkValue(SPORK_8_XFSNODE_PAYMENT_ENFORCEMENT) && !IsBlockPayeeValid(txNew, pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(): couldn't find xfsnode or superblock payments"),
    ...
    LogPrint("bench", "    - XFS writing: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeIndex * 0.000001);
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams &chainParams) {
    ...
    xfsnodeSync.UpdatedBlockTip(chainActive.Tip());
...
CAmount GetXFSnodePayment(const Consensus::Params &params, bool fMTP,int nHeight) {
if(nHeight > Params().GetConsensus().nXFSnodePaymentsStartBlock)
{
    //Give 80 % to masternode
    return GetBlockSubsidy(nHeight,params) * 0.7;
}
    //No reward before xfsnodepaymentsstartblock
    return 0;
}
...
bool CheckBlock(const CBlock &block, CValidationState &state,
    ...
    try {
        ...
        if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
            ...
            BOOST_FOREACH(const CTransaction& tx, block.vtx) {
                ...
                BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                    ...
                    if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                        ...
                        LOCK(cs_main);
                        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                        return state.DoS(0, error("CheckBlock(XFS): transaction %s conflicts with transaction lock %s",
            ...
            LogPrintf("CheckBlock(XFS): spork is off, skipping transaction locking checks\n");
        }
...
bool ProcessNewBlock(CValidationState &state, const CChainParams &chainparams, CNode *pfrom, const CBlock *pblock,
    ...
    xfsnodeSync.GetBlockchainSynced(true);
...
bool static AlreadyHave(const CInv &inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main) {
    switch (inv.type) {
        ...
        /*
            XFS Related Inventory Messages
        ...
        */
        ...
        case MSG_XFSNODE_PAYMENT_VOTE:
            return mnpayments.mapXFSnodePaymentVotes.count(inv.hash);

        case MSG_XFSNODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && mnpayments.mapXFSnodeBlocks.find(mi->second->nHeight) != mnpayments.mapXFSnodeBlocks.end();
        }

        case MSG_XFSNODE_ANNOUNCE:
            return mnodeman.mapSeenXFSnodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);

        case MSG_XFSNODE_PING:
            return mnodeman.mapSeenXFSnodePing.count(inv.hash);

        case MSG_DSTX:
            return mapDarksendBroadcastTxes.count(inv.hash);

        case MSG_XFSNODE_VERIFY:
            return mnodeman.mapSeenXFSnodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK ||
                ...
                if (!pushed && inv.type == MSG_XFSNODE_PAYMENT_VOTE) {
                    if(mnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnpayments.mapXFSnodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::XFSNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_XFSNODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapXFSnodeBlocks);
                    if (mi != mapBlockIndex.end() && mnpayments.mapXFSnodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CXFSnodePayee& payee, mnpayments.mapXFSnodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(mnpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << mnpayments.mapXFSnodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::XFSNODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_XFSNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenXFSnodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenXFSnodeBroadcast[inv.hash].second;
                        pfrom->PushMessage(NetMsgType::MNANNOUNCE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_XFSNODE_PING) {
                    if(mnodeman.mapSeenXFSnodePing.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenXFSnodePing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_XFSNODE_VERIFY) {
                    if(mnodeman.mapSeenXFSnodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenXFSnodeVerification[inv.hash];
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (!vRecv.empty()) {
            ...
            if(fCheckClientVersion){
                int parsedVersion[4];
                if (sscanf(pfrom->cleanSubVer.c_str(), "/XFS:%2d.%2d.%2d.%2d/",
...
//        vRecv >> tx;
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            CXFSnode *pmn = mnodeman.Find(dstx.vin);
            if (pmn == NULL) {
                LogPrint("privatesend", "DSTX -- Can't find xfsnode %s to verify %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if (!pmn->fAllowMixingTx) {
                LogPrint("privatesend", "DSTX -- XFSnode %s is sending too many transactions %s\n",
                         dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
            ...
            if (!dstx.CheckSignature(pmn->pubKeyXFSnode)) {
...
//        LogPrintf("Main.cpp ProcessMessage() strCommand=%s\n", strCommand);
        ...
        if (found) {
            ...
            xfsnodeSync.ProcessMessage(pfrom, strCommand, vRecv);
        } else {
            // Ignore unknown commands for extensibility
