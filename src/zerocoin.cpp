...
bool CheckZerocoinFoundersInputs(const CTransaction &tx, CValidationState &state, const Consensus::Params &params, int nHeight, bool fMTP) {
        ...
        if (nHeight >= params.nXFSnodePaymentsStartBlock) {
            CAmount xfsnodePayment = GetXFSnodePayment(params, fMTP,nHeight);
            BOOST_FOREACH(const CTxOut &output, tx.vout) {
                if (xfsnodePayment == output.nValue) {
                    total_payment_tx = total_payment_tx + 1;
                }
            }

            bool validXFSnodePayment;

            if (nHeight > params.nXFSnodePaymentsBugFixedAtBlock) {
                if (!xfsnodeSync.IsSynced()) {
                    validXFSnodePayment = true;
                } else {
                    validXFSnodePayment = mnpayments.IsTransactionValid(tx, nHeight, fMTP);
                }
            } else {
                validXFSnodePayment = total_payment_tx <= 1;
            }

            if (!validXFSnodePayment) {
                return state.DoS(100, false, REJECT_INVALID_XFSNODE_PAYMENT,
                                 "CTransaction::CheckTransaction() : invalid xfsnode payment");
            }
