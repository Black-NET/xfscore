...
def print_welcome():
    ...
    print("")
    print("")
    print_info("xfs masternode installer v1.0")
    print("")
    print("")
    print("")
...
def secure_server():
    ...
    run_command("ufw allow 8558")
    run_command("ufw default deny incoming")
    run_command("ufw default allow outgoing")
    run_command("ufw --force enable")

def checkdaemon():
    return os.path.isfile('/usr/local/bin/xfsd')

# Helper functions for automating updating and installing daemon
def getlatestrelease():
    r = requests.get(url='https://api.github.com/repos/xfs/Index/releases/latest')
...
def installdaemon(fupdate):
    os.system('su - mn1 -c "{}" '.format('xfs-cli stop &> /dev/null'))
    ...
    if fupdate:
        ...
        os.system('su - mn1 -c "{}" '.format('xfsd -daemon &> /dev/null'))
    else:
...
def setup_first_masternode():
    ...
    print_info("Open your desktop wallet config file (%appdata%/xfs/xfs.conf) and copy your rpc username and password! If it is not there create one! E.g.:\n\trpcuser=[SomeUserName]\n\trpcpassword=[DifficultAndLongPassword]")
    global rpc_username
    global rpc_password
    rpc_username = input("rpcuser: ")
    rpc_password = input("rpcpassword: ")

    print_info("Open your wallet console (Help => Debug window => Console) and create a new masternode private key: xfsnode genkey")
    masternode_priv_key = input("xfsnodeprivkey: ")
...
""".format(rpc_username, rpc_password, masternode_priv_key)
    ...
    run_command('su - mn1 -c "mkdir /home/mn1/.xfs"')
    run_command('su - mn1 -c "touch /home/mn1/.xfs/xfs.conf"')
    run_command('su - mn1 -c "touch /home/mn1/.xfs/exodus.log"')
    run_command('su - mn1 -c "touch /home/mn1/.xfs/debug.log"')
    f = open('/home/mn1/.xfs/xfs.conf', 'w')
    f.write(config)
    f.close()
...
def setup_xth_masternode(xth):
    ...
    run_command("rm -rf /home/mn{}/.xfs/".format(xth))

    print_info('Copying wallet data from the first masternode...')
    run_command("cp -rf /home/mn1/.xfs /home/mn{}/".format(xth))
    run_command("sudo chown -R mn{}:mn{} /home/mn{}/.xfs".format(xth, xth, xth))
    run_command("rm /home/mn{}/.xfs/peers.dat &> /dev/null".format(xth))
    run_command("rm /home/mn{}/.xfs/wallet.dat &> /dev/null".format(xth))
...
""".format(rpc_username, rpc_password, BASE_RPC_PORT + xth - 1, BASE_PORT + xth - 1, masternode_priv_key)
    ...
    print_info("Saving config file...")
    f = open('/home/mn{}/.xfs/xfs.conf'.format(xth), 'w')
    f.write(config)
    f.close()
    
    autostart_masternode('mn'+str(xth))
    os.system('su - mn{} -c "{}" '.format(xth, 'xfsd  -daemon &> /dev/null'))
    print_warning("Masternode started syncing in the background...")
...
IP: {}
Private key: {}
Transaction ID: [5k IDX deposit transaction id. 'xfsnode outputs']
Transaction index: [5k IDX deposit transaction index. 'xfsnode outputs']
mnconf line :
{} {} {} txhash txindex
--------------------------------------------------
"""
    ...
    for idx, val in enumerate(PRIVATE_KEYS):
        SERVER_IP_STRING = SERVER_IP + ":".encode('utf-8') + str(8558).encode('utf-8')
...
\tthe 'xfs-cli getinfo' to get actual block number. Go to
